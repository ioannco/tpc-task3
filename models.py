...
class NERDataset(Dataset):
    ...
    def __init__(self, data, token2idx, label2idx):
        self.data = data  # [(tokens, labels), ...]
        self.token2idx = token2idx
        self.label2idx = label2idx

    def __getitem__(self, idx):
        tokens, labels = self.data[idx]
        tokens_idx = [self.token2idx.get(t, self.token2idx["<PAD>"]) for t in tokens]
        labels_idx = [self.label2idx[l] for l in labels]
        return torch.tensor(tokens_idx), torch.tensor(labels_idx)
...
class BiLSTM_CRF(nn.Module):
    """
    BiLSTM-CRF Network for Named Entity Recognition (NER).

    This model combines a BiLSTM network for feature extraction and
    a Conditional Random Field (CRF) for structured prediction.

    The BiLSTM processes input token embeddings to capture context
    from both directions (left-to-right and right-to-left).
    The CRF layer ensures that the predicted labels follow
    a valid sequential structure.

    Args:
        vocab_size (int): Size of the vocabulary (number of unique tokens).
        tagset_size (int): Number of tags (BIO tags or other labels).
        embedding_dim (int): Dimensionality of the token embeddings.
        hidden_dim (int): Dimensionality of the LSTM hidden states.
        pad_idx (int): Index used for padding tokens in the input sequences.
    """
    ...
    def forward(self, tokens, tags=None, mask=None):
        ...
        Args:
            tokens (Tensor): Input token indices with shape (batch_size, seq_length).
                             Each token index corresponds to its position in the vocabulary.
            tags (Tensor, optional): Ground truth tag indices with shape (batch_size, seq_length).
                                     Used for computing the CRF loss during training.
            mask (Tensor, optional): Binary mask indicating valid tokens (non-padding)
                                     with shape (batch_size, seq_length).

        Returns:
            - If `tags` is provided (training mode):
                Tensor: Negative log-likelihood loss for the batch (scalar).
            - If `tags` is not provided (inference mode):
                List[List[int]]: Predicted tag sequences for each input sequence.
                                 Each list contains tag indices for the corresponding tokens.
        """
        # Compute embeddings for input tokens
        embeds = self.embedding(tokens)  # Shape: (batch_size, seq_length, embedding_dim)

        # Pass embeddings through the BiLSTM
        lstm_out, _ = self.lstm(embeds)  # Shape: (batch_size, seq_length, hidden_dim)
        lstm_out = self.dropout(lstm_out)

        # Compute emissions (logits for each tag)
        emissions = self.fc(lstm_out)  # Shape: (batch_size, seq_length, tagset_size)

        # Transpose emissions and mask for CRF compatibility
        emissions = emissions.transpose(0, 1)  # Shape: (seq_length, batch_size, tagset_size)
        mask = mask.transpose(0, 1)  # Shape: (seq_length, batch_size)

        if tags is not None:
            # Training mode: Compute CRF loss
            tags = tags.transpose(0, 1)  # Shape: (seq_length, batch_size)
            loss = -self.crf(emissions, tags, mask)
            return loss
        else:
            # Inference mode: Decode the best tag sequence using Viterbi algorithm
